...
- Redirectovati neulogovane korisnike sa `/admin` na `/login` (Supabase Auth session)
- Matcher pokriva samo `/admin/:path*` — javni URL je namerno otvoren (demo se deli sa potencijalnim klijentima)
- Ne primenjivati auth check na `/api/webhook/whatsapp` (Twilio ne šalje Supabase session) — signature verifikacija (§5) je jedina linija odbrane, pa mora biti striktna
- `/api/mcp` se štiti `x-mcp-secret` header-om u route handler-u, ne preko proxy-ja
...
| 10 | ~~Basic-auth zavesa na javnom URL-u~~ — uklonjena; demo URL je namerno javan, `/admin` štiti Supabase Auth | N/A |
